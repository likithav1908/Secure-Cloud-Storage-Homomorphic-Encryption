...
<%
    ...
    try {
        ...
        // Paillier encryption requires a public key for encryption
        BigInteger n = new BigInteger("9574953798573056"); // Example value for n (part of Paillier keys)
        BigInteger g = new BigInteger("2133"); // Example value for g (part of Paillier keys)

        // Encrypt the AES key using Paillier
        BigInteger aesKeyBigInt = new BigInteger(secretKey.getEncoded()); // Convert AES key to BigInteger
        BigInteger r = new BigInteger("4398253"); // Random value r for encryption

        // Paillier encryption formula: c = g^m * r^n mod n^2
        BigInteger c = g.modPow(aesKeyBigInt, n.multiply(n)).multiply(r.modPow(n, n.multiply(n))).mod(n.multiply(n));

        // Encode the Paillier-encrypted AES key in Base64 for easier display
        String encryptedAESKeyBase64 = Base64.getEncoder().encodeToString(c.toByteArray());
        ...
        encryptedData = "Encrypted File Data (Base64): " + encryptedFileBase64 + "<br/>" +
                        "Encrypted AES Key using Paillier (Base64): " + encryptedAESKeyBase64;
...
<html>
    <head>
        <title>Paillier and AES Encryption</title>
        <style>
            body {
